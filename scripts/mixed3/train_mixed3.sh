...
# === Configuration ===
DATASET_NAME=expert_${DOM1}-${MIXTURE_THRESHOLD},expert_${DOM2}-${MIXTURE_THRESHOLD},expert_${DOM3}-${MIXTURE_THRESHOLD}
...
MIXTURE_ID="mixed_${DOM1}-${N1}++${DOM2}-${N2}++${DOM3}-${N3}"
...
if [ $status -eq 0 ]; then
  ...
  # LOG_PATH=logs/${EXP_NAME}/export_mixed3/%A_%x/%A_%a_%x
  # sbatch --job-name=export_${BASE_MODEL}_${SFT_STRATEGY}_${DOM1}_${DOM2}_${DOM3} \
